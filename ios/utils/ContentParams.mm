...
+ (NSDictionary<NSString*, NSString*> *)parseHeaderFromString:(NSString *)headerString {
   if (!headerString || headerString.length == 0) return @{};
   ...
   for (NSString *part in parts) {
       ...
       NSString *key =
           [[kv[0] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]] copy];

       NSString *value =
           [[[kv subarrayWithRange:NSMakeRange(1, kv.count - 1)]
