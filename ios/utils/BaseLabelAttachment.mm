...
- (CGRect)contentRectForContainer:(CGSize)containerSize {
    return CGRectMake(
        ...
        containerSize.height - self.margin.top - self.margin.bottom
    );
}
...
- (void)drawBorderInRect:(CGRect)rect context:(CGContextRef)ctx {
    ...
    CGRect borderRect = CGRectInset(rect, self.borderWidth / 2, self.borderWidth / 2);
    UIBezierPath *path =
...
- (UIImage *)imageForBounds:(CGRect)bounds textContainer:(NSTextContainer *)textContainer characterIndex:(NSUInteger)charIndex {
