...
- (UIImage *)renderAttachmentInSize:(CGSize)containerSize {
    ...
    CGFloat imgWidth = (self.imageWidth > 0)
        ? self.imageWidth
        : lineWidth * 0.30;
    ...
    CGFloat textAreaWidth =
        lineWidth - imgWidth - spacing;

    CGSize canvas = CGSizeMake(
        lineWidth + self.margin.left + self.margin.right,
    ...
    if (self.contentImage && !self.isLoading) {
        ...
        ImageResizeMode mode = [ImageLayoutUtils resizeModeFromString:self.imageResizeMode];

        CGRect target = [ImageLayoutUtils rectForImage:self.contentImage inRect:imageRect resizeMode:mode];
    ...
    CGRect textRect = CGRectMake(
        ...
        textAreaWidth - self.inset.left - self.inset.right,
        textSize.height
    );
