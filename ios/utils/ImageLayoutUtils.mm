...
+ (ImageResizeMode)resizeModeFromString:(NSString *)name {
    ...
    if ([m isEqualToString:@"cover"])       return ImageResizeModeCover;
    if ([m isEqualToString:@"contain"])     return ImageResizeModeContain;
    if ([m isEqualToString:@"fill"])        return ImageResizeModeFill;
    if ([m isEqualToString:@"stretch"])     return ImageResizeModeFill;
    if ([m isEqualToString:@"none"])        return ImageResizeModeNone;
    if ([m isEqualToString:@"center"])      return ImageResizeModeNone;
    if ([m isEqualToString:@"scale-down"])  return ImageResizeModeScaleDown;
    ...
    switch (mode) {
        ...
        case ImageResizeModeNone:
            return CGRectMake(slot.origin.x + (slotW - img.width)/2,
                              slot.origin.y + (slotH - img.height)/2,
